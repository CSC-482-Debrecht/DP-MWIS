	...
	for (int j = 0; j < NUM_EVENTS + 1; ++j) {
		for (int k = j; k < NUM_EVENTS + 1; ++k) {
			...
			for (int mask = (1 << k) - 1; mask >= 0; --mask) {
				...
				option3 = (OPT[c[j]][k-1][(mask &~ (1UL << (k-j))*(j>0)) >> min(k-c[j], 1)] + events[j].getValue())*((mask >> (k-j)) & 1U); // Check if we've used j yet. The bit of interest is a distance k-j from the rightmost bit.
